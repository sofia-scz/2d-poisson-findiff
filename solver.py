...
def setup_main(dx, dy, n, m, xi, F):
    ...
    M_coef, V_indep = np.zeros((Mdim, Mdim)), np.zeros(
        Mdim)  # crear mat y v indep
    ...
    for i in range(1, n):      # barrer sobre los puntos internos de la grilla
        for j in range(1, m):
            ...
            # coefs celdas ady hori
            M_coef[k, k+(m+1)], M_coef[k, k-(m+1)] = cx, cx
...
def solve_poisson(x0, xf, y0, yf, n, m, xi, F):
    # definir constantes
    ...
    # solucion en formato k (cambiar esto por linalg.solve?)
    sol = np.linalg.solve(M_coef, V_indep)
